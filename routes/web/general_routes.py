...
def rutas():
    try:
        ...
        # Preparar las rutas para la tabla
        lista_rutas = []
        for ruta in rutas:
            lista_rutas.append({
                "id": str(ruta["_id"]),
...
@web_bp.route('/crear_ruta')
def crear_ruta():
    return render_template('crear_ruta.html')

@web_bp.route('/guardar_ruta', methods=['POST'])
def guardar_ruta():
    try:
        data = request.get_json()
        nombre = data.get('nombre')
        geojson_ruta = data.get('ruta')

        if not nombre or not geojson_ruta:
            return jsonify({"success": False, "message": "Nombre o ruta no proporcionados."}), 400

        db = get_db()

        # Convertir a FeatureCollection si no lo es
        if geojson_ruta.get('type') != 'FeatureCollection':
            geojson_ruta = {
                "type": "FeatureCollection",
                "features": [geojson_ruta]
            }

        # Verificar si ya existe una ruta con el mismo nombre
        existing_route = db['rutas'].find_one({"nombre": nombre})
        if existing_route:
            return jsonify({"success": False, "message": f"Ya existe una ruta con el nombre '{nombre}'."}), 400
        ...
        return jsonify({"success": True, "message": "Ruta guardada exitosamente."}), 201

    except Exception as e:
        print(f"Error al guardar la ruta: {e}")
        return jsonify({"success": False, "message": "Error al guardar la ruta."}), 500


@web_bp.route('/ver_ruta/<string:ruta_id>', methods=['GET'])
def ver_ruta(ruta_id):
    try:
        ...
        if not ruta:
            ...
            return redirect(url_for('web_bp.rutas'))

        # Asegurar que la ruta es un FeatureCollection
        geojson = ruta.get("ruta", {})
        ...
        return render_template(
            'ver_ruta.html',
            ruta_nombre=ruta.get("nombre", "Ruta sin nombre"),
            geojson=geojson
        )
    except Exception as e:
        print(f"Error al cargar la ruta: {e}")
        flash("Error interno al cargar la ruta.", "error")
        return redirect(url_for('web_bp.rutas'))


@web_bp.route('/editar_ruta/<string:ruta_id>', methods=['GET', 'POST'])
def editar_ruta(ruta_id):
    ...
    if request.method == 'POST':
        try:
            ...
            if not nuevo_nombre or not nueva_ruta:
                return jsonify({"success": False, "message": "Nombre o ruta no proporcionados."}), 400

            # Convertir a FeatureCollection si no lo es
            if nueva_ruta.get('type') != 'FeatureCollection':
                nueva_ruta = {
                    "type": "FeatureCollection",
                    "features": [nueva_ruta]
                }
            ...
            ruta_existente = db['rutas'].find_one({"nombre": nuevo_nombre, "_id": {"$ne": ObjectId(ruta_id)}})
            if ruta_existente:
                return jsonify({"success": False, "message": f"Ya existe una ruta con el nombre '{nuevo_nombre}'."}), 400
            ...
            return jsonify({"success": True, "message": "Ruta actualizada exitosamente."}), 200

        except Exception as e:
            print(f"Error al editar la ruta: {e}")
            return jsonify({"success": False, "message": "Error al actualizar la ruta."}), 500
